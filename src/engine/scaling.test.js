...
describe('Scaling Engine', () => {
  describe('calculateBaseReps', () => {
    it('should scale run distances based on duration', () => {
      ...
      expect(calculateBaseReps(runEx, 'Rx', EXTREME + 1)).toBe(
        SCALING_CONSTANTS.RUN_DISTANCES.EXTREME
      );
      expect(calculateBaseReps(runEx, 'Rx', EXTRA_LONG + 1)).toBe(
        SCALING_CONSTANTS.RUN_DISTANCES.EXTRA_LONG
      );
      expect(calculateBaseReps(runEx, 'Rx', SHORT - 1)).toBe(SCALING_CONSTANTS.RUN_DISTANCES.SHORT);
      expect(calculateBaseReps(runEx, 'Rx', SHORT + 5)).toBe(
    ...
    it('should scale Machine equipment based on duration', () => {
      ...
      expect(calculateBaseReps(machineEx, 'Rx', EXTREME + 1)).toBe(
        SCALING_CONSTANTS.MACHINE_REPS.EXTREME
      );
      expect(calculateBaseReps(machineEx, 'Rx', EXTRA_LONG + 1)).toBe(
        SCALING_CONSTANTS.MACHINE_REPS.EXTRA_LONG
      );
      expect(calculateBaseReps(machineEx, 'Rx', SHORT - 1)).toBe(
        SCALING_CONSTANTS.MACHINE_REPS.SHORT
      );
      expect(calculateBaseReps(machineEx, 'Rx', SHORT + 5)).toBe(
        SCALING_CONSTANTS.MACHINE_REPS.DEFAULT
      );
    });
    ...
    it('should scale reps based on intensity', () => {
      ...
      expect(calculateBaseReps(veryHighIntEx, 'Rx', 20)).toBe(
        SCALING_CONSTANTS.INTENSITY_REPS.VeryHigh
      );
    ...
    it('should scale jump rope exercises', () => {
      ...
      expect(calculateBaseReps({ id: 'su' }, 'Rx', 20)).toBe(
        SCALING_CONSTANTS.SPECIAL_REPS['Single Unders']
      );
    });
    ...
    it('should apply beginner multiplier and handle jump rope for beginners', () => {
      ...
      const expectedBeginnerReps = Math.ceil(
        SCALING_CONSTANTS.DEFAULT_REPS * SCALING_CONSTANTS.BEGINNER_MULTIPLIER
      );
      expect(calculateBaseReps(normalEx, 'Beginner', 20)).toBe(expectedBeginnerReps);
      expect(calculateBaseReps(duEx, 'Beginner', 20)).toBe(
        SCALING_CONSTANTS.SPECIAL_REPS.Beginner_Jump
      );
    });
  });
  ...
  describe('getReps', () => {
    it('should retrieve strategy and calculate scaled reps', () => {
      ...
      expect(mockScaleReps).toHaveBeenCalledWith(
        SCALING_CONSTANTS.DEFAULT_REPS,
        exercise,
        'Rx',
        20
      );
