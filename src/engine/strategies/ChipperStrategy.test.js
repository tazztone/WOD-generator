...
describe('ChipperStrategy', () => {
  ...
  describe('scaleReps', () => {
    ...
    it('should handle Machine calories scaling', () => {
      expect(ChipperStrategy.scaleReps(10, { equipment: 'Machine', id: 'row' }, 'Normal', 20)).toBe(
        '40/30 cal'
      );
    });
    ...
    it('should apply multiplier 2 for VeryHigh intensity or muscle_up', () => {
      expect(
        ChipperStrategy.scaleReps(3, { intensity: 'VeryHigh', id: 'something' }, 'Normal', 20)
      ).toBe(6);
      expect(
        ChipperStrategy.scaleReps(3, { intensity: 'High', id: 'bar_muscle_up' }, 'Normal', 20)
      ).toBe(6);
    });

    it('should apply multiplier 3 for High intensity or pullup/hspu', () => {
      expect(
        ChipperStrategy.scaleReps(5, { intensity: 'High', id: 'something' }, 'Normal', 20)
      ).toBe(15);
      expect(
        ChipperStrategy.scaleReps(5, { intensity: 'Moderate', id: 'strict_pullup' }, 'Normal', 20)
      ).toBe(15);
      expect(
        ChipperStrategy.scaleReps(5, { intensity: 'Moderate', id: 'strict_hspu' }, 'Normal', 20)
      ).toBe(15);
    });

    it('should apply default multiplier 4 for normal exercises', () => {
      expect(
        ChipperStrategy.scaleReps(10, { intensity: 'Moderate', id: 'pushup' }, 'Normal', 20)
      ).toBe(40);
    });

    it('should cap VeryHigh intensity reps at 8', () => {
      expect(
        ChipperStrategy.scaleReps(5, { intensity: 'VeryHigh', id: 'something' }, 'Normal', 20)
      ).toBe(8); // 5 * 2 = 10 -> capped at 8
    });

    it('should cap standard exercise reps at 60', () => {
      expect(
        ChipperStrategy.scaleReps(20, { intensity: 'Moderate', id: 'pushup' }, 'Normal', 20)
      ).toBe(60); // 20 * 4 = 80 -> capped at 60
    });

    it('should set double unders (du) to exactly 100', () => {
      expect(ChipperStrategy.scaleReps(20, { intensity: 'Moderate', id: 'du' }, 'Normal', 20)).toBe(
        100
      );
    });

    it('should set single unders (su) to exactly 150', () => {
      expect(ChipperStrategy.scaleReps(20, { intensity: 'Moderate', id: 'su' }, 'Normal', 20)).toBe(
        150
      );
    });
  });
