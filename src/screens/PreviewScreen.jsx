...
export const PreviewScreen = ({
  ...
  const handleShare = () => {
    ...
    if (workout.strength) {
      const strengthName = workout.strength.nameKey
        ? LOCALES.strength.names[workout.strength.nameKey][lang] || workout.strength.nameKey
        : workout.strength.name;
      const strengthNote = workout.strength.noteKey
        ? LOCALES.strength.notes[workout.strength.noteKey][lang] || workout.strength.noteKey
        : workout.strength.notes;
    ...
    text += workout.exercises
      .map((e) => `${formatReps(e.reps, e.exercise)} ${getExerciseName(e.exercise, lang)}`)
      .join('\n');
  ...
  const getSwapCandidates = (index) => {
    ...
    return validExercises.filter(
      (ex) => ex.pattern === current.exercise.pattern && !existingExerciseIds.has(ex.id)
    );
  };
  ...
  return (
    <div className="flex flex-col h-full animate-zoom-in duration-300 relative">
      ...
      {swapModal.show && (
        <div className="absolute inset-0 z-50 bg-slate-900/95 backdrop-blur flex flex-col p-4 animate-fade-in duration-200">
          <div className="flex justify-between items-center mb-2">
            ...
            <button
              onClick={() => setSwapModal({ show: false, index: -1 })}
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pb-4">
            {getSwapCandidates(swapModal.index).map((ex) => (
              <button
                key={ex.id}
                onClick={() => {
                  onManualSwap(swapModal.index, ex.id);
                  setSwapModal({ show: false, index: -1 });
                }}
              ...
              >
                <div className="relative z-10 flex justify-between items-center">
                  <div>
                    <span className="font-bold text-slate-200 block group-hover:text-emerald-400 text-lg mb-1">
                    ...
                    <div className="flex gap-2 items-center text-[10px] text-slate-500 uppercase tracking-widest">
                      ...
                      {ex.intensity === 'VeryHigh' && (
                        <span className="flex items-center gap-1 text-rose-500">
                          <Flame size={10} /> Intense
                        </span>
                      )}
                    </div>
                  </div>
      ...
      <div className="flex-1 px-5 pt-3 overflow-y-auto pb-32">
        <button
          onClick={onBack}
          className="mb-4 flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-white uppercase tracking-widest transition-colors"
        >
        ...
        {workout.strength && (
          <div className="mb-6 bg-slate-800/40 border-l-4 border-purple-500 p-4 rounded-r-xl relative">
            <button
              ...
              className="absolute top-2 right-2 text-slate-600 hover:text-purple-400 transition-colors p-2"
            >
              <Info size={16} />
            </button>
            ...
            <h3 className="text-xl font-black text-white">
              {workout.strength.nameKey
                ? LOCALES.strength.names[workout.strength.nameKey][lang] || workout.strength.nameKey
                : workout.strength.name}
            </h3>
            <p className="text-sm text-slate-400 mt-1">
              {workout.strength.sets} —{' '}
              {workout.strength.noteKey
                ? LOCALES.strength.notes[workout.strength.noteKey][lang] || workout.strength.noteKey
                : workout.strength.notes}
            </p>
          </div>
        ...
        <div className="flex justify-between items-end mb-2">
          <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest flex items-center gap-1">
            ...
            <button
              ...
              className="ml-1 text-slate-600 hover:text-emerald-500"
            >
              <Info size={12} />
            </button>
          </span>
          <button
            onClick={handleShare}
            className="text-xs flex items-center gap-1 text-slate-500 hover:text-emerald-400 transition-colors mr-3"
          >
            {copied ? <CheckCircle size={12} /> : <Share2 size={12} />}{' '}
            {copied ? pt.copied : pt.share}
          </button>
          <button
            onClick={onToggleSave}
            className={`text-xs flex items-center gap-1 transition-colors ${isSaved ? 'text-emerald-400' : 'text-slate-500 hover:text-emerald-400'}`}
          >
            <Star size={12} fill={isSaved ? 'currentColor' : 'none'} />{' '}
            {isSaved ? pt.saved : pt.save}
          </button>
          <button
            onClick={handleReroll}
            className={`text-xs flex items-center gap-1 transition-all duration-300 ${isRerollConfirmed ? 'text-rose-500 font-black animate-pulse' : 'text-slate-500 hover:text-emerald-400'}`}
          >
            <Dice5
              size={12}
              className={isRerollConfirmed ? 'rotate-180 transition-transform duration-500' : ''}
            />{' '}
            {isRerollConfirmed ? pt.rerollConfirm : pt.reroll}
          </button>
        </div>
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700/50 rounded-2xl p-5 mb-5 shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter mb-1">
              {workout.isPartner && (
                <span className="text-emerald-500 not-italic mr-2">Partner</span>
              )}
              {workout.template}
            </h2>
            <div className="flex items-center gap-3 text-slate-400 text-xs font-mono font-bold">
              <span className="flex items-center gap-1">
                <Clock size={12} /> {config.duration} {pt.min}
              </span>
              {workout.rounds && (
                <span>
                  • {workout.rounds} {pt.rounds}
                </span>
              )}
            </div>
              ...
              <div className="mt-4 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl relative">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm font-bold text-slate-200">
                    {getExerciseName(workout.buyIn.exercise, lang)}
                  </span>
            ...
            <div className="mt-6 space-y-3">
              {workout.exercises.map((item, idx) => (
                <div
                  key={idx}
                  onClick={() => setSwapModal({ show: true, index: idx })}
                  className="flex items-center justify-between group cursor-pointer bg-slate-800/30 p-2 -mx-2 rounded-lg hover:bg-slate-800 transition-colors"
                >
                  <div className="flex items-baseline gap-3">
                    ...
                    <span className="text-slate-200 font-bold group-hover:text-emerald-300 transition-colors">
                      {getExerciseName(item.exercise, lang)}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] text-slate-600 font-bold uppercase tracking-wider group-hover:text-slate-500">
                      {item.exercise.pattern}
                    </span>
                    <RefreshCw
                      size={12}
                      className="text-slate-500 group-hover:text-emerald-500 opacity-50 group-hover:opacity-100 transition-all"
                    />
                  </div>
                </div>
              ))}
        ...
        {workout.warmup && workout.warmup.length > 0 && (
          <div className="bg-slate-900/50 border border-dashed border-slate-800 rounded-xl p-4 relative">
            <button
              ...
              className="absolute top-2 right-2 text-slate-700 hover:text-slate-400 transition-colors p-2"
            >
              <Info size={14} />
            </button>
