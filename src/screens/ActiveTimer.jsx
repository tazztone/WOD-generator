...
export const ActiveTimer = ({ workout, onExit, onSave, lang }) => {
  const { config, setConfig, modalOpen, setModalOpen } = useSettings();
  ...
  useWakeLock();
  const { status, timeLeft, totalTime, currentRound, setCurrentRound, isPaused, setIsPaused } =
    useTimer(workout, lang, audioSettings);
  ...
  return (
    <div
      className={`flex flex-col h-full relative overflow-hidden transition-colors duration-500 ${status === 'rest' ? 'bg-blue-900' : 'bg-slate-900'}`}
    >
          ...
          <div className="bg-slate-800/95 backdrop-blur-xl border border-slate-700 rounded-2xl shadow-2xl p-4 overflow-hidden">
            ...
            <div className="space-y-3">
              {Object.entries(t.audioSettings).map(([key, label]) => (
                ...
                >
                  <span
                    className={`text-sm font-bold transition-colors ${config.audioSettings[key] ? 'text-white' : 'text-slate-500'}`}
                  >
                    {label}
                  </span>
                  <div
                    className={`w-10 h-5 rounded-full relative transition-colors ${config.audioSettings[key] ? 'bg-emerald-500' : 'bg-slate-600'}`}
                  >
                    <div
                      className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-transform ${config.audioSettings[key] ? 'translate-x-6' : 'translate-x-1'}`}
                    />
                  </div>
                </button>
              ))}
      ...
      >
        <button
          ...
          className="p-2 bg-slate-800/50 rounded-full text-slate-400"
      ...
      <div className="flex-1 flex flex-col items-center justify-center z-10">
        <span
          className={`text-[15vw] font-black font-mono leading-none tracking-tighter ${status === 'rest' ? 'text-blue-300' : status === 'pre' ? 'text-yellow-400' : 'text-white'}`}
        >
          {status === 'pre'
            ? timeLeft
            : workout.template === 'EMOM' || workout.template === 'Tabata'
              ? timeLeft
              : formatTime(timeLeft)}
        </span>
          ...
          <div className="mt-8 flex flex-col items-center">
            ...
            <div className="flex items-center gap-6 mt-2">
              <button
                ...
                className="p-3 bg-slate-800 rounded-full text-slate-400 hover:bg-slate-700"
      ...
      <div className="p-6 bg-slate-950/80 backdrop-blur-md border-t border-slate-800 z-10">
        ...
        <div className="text-sm text-slate-300 leading-relaxed font-medium">{exerciseList}</div>
      </div>
      ...
      {modalOpen && (
        <div className="absolute inset-0 z-[100] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-6 animate-fade-in">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 w-full max-w-sm shadow-2xl animate-zoom-in">
            ...
            <h2 className="text-2xl font-black text-white italic text-center mb-2">
              {st.confirmExit}
            </h2>
